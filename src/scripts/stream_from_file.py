...
def main(video_directory: str, number_of_streams: int) -> None:
    """
    Main function to set up the RTSP server and stream videos.
    """
...
def find_video_files_in_directory(directory: str, limit: int) -> list:
    """
    Gets first 'limit' number of videos from a directory with .mp4 or .webm extensions.
    """
...
def create_server_config_file(directory: str) -> str:
    """
    Creates a YAML config file for the RTSP server.
    """
...
def run_rtsp_server(config_path: str) -> None:
    """
    Starts the RTSP server using Docker.
    """
    existing_containers = subprocess.getoutput("docker ps -a --format '{{.Names}}'")
    if "rtsp_server" in existing_containers:
        run_command(["docker", "start", "rtsp_server"])
    else:
        command = [
            "docker", "run", "-d", "--name", "rtsp_server", "-v",
...
def stop_rtsp_server() -> None:
    """
    Stops and removes the RTSP server container.
    """
    run_command(["docker", "stop", "rtsp_server"])
    run_command(["docker", "rm", "rtsp_server"])

def stream_videos(video_files: list) -> None:
    """
    Streams video files in separate threads.
    """
    threads = []
...
def stream_video_to_url(video_path: str, stream_url: str) -> Thread:
    """
    Uses FFmpeg to stream a video file to a given RTSP URL.
    """
    command = [
...
def run_command_in_thread(command: list) -> Thread:
    """
    Runs a command in a separate thread and logs errors if any.
    """
    def target():
...
def run_command(command: list) -> int:
    """
    Runs a shell command and returns the exit status.
    """
